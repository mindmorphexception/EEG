function [modules, modularities] = modularity_louvain_und_history(connectionMatrix, oldModules, gamma)
...
if ~exist('oldModules', 'var')
    oldModules = {[],1:nrNodes};
end

hierarchyIndex = 2;                                          %hierarchy index
nrNodesInHierarchy = nrNodes;                                %number of nodes in hierarchy
modules = { [],  oldModules };                               %hierarchical module assignments
...
while modularities{hierarchyIndex}-modularities{hierarchyIndex-1}>1e-10
    ...
    moduleAssignment = 1:nrNodesInHierarchy;              %initial module assignments
    ...
    newConnectionMatrix = zeros(nrNodesInHierarchy);               %new weighted matrix
