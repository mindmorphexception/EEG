...
ms = {'smallworldness','modspanmean', 'meanclustering', 'stdclustering', 'pathlen', 'meanbetweenness', 'stdbetweenness', 'modularity', 'meanparticipation', 'stdparticipation'...
    };
titles = {'Small-world-ness','Modular span', 'Median clustering', 'SD clustering', 'Characteristic path length', 'Median betweenness', 'SD betweenness', 'Modularity', 'Median participation', 'SD participation',...
    };
...
%             PlotBarsSorted(otherparams);
        ...
        allMeasureNames{end+1} = nicetitle;
...
%         
%         %if(spearman.pval <= 0.11 || lm.pval <= 0.11)
%             
%             % plot
...
%             hgexport(gcf, [folder nicetitle '.png'], myStyle);
%             
%             % display
...
%         %end
        ...
        filename = ['sd-' ms{j} '-' fr{i} ];
        nicetitle = ['SD ' titles{j} ' - ' fr{i}];
        if (strcmp(nicetitle(1:5),'SD SD'))
            continue;
        end
        load([folder filename '.mat'])    
